    ...
    try {
        const attempt = await prisma.$transaction(async (tx) => {
            ...
            for (const response of responses) {
                ...
                if (!question) {
                    throw httpError(400, `Question ${response.questionId} does not belong to quiz ${quizId}.`);
                }
                const option = question.options.find((item) => item.id === response.optionId);
                if (!option) {
                    throw httpError(400, `Option ${response.optionId} does not belong to question ${response.questionId}.`);
                }
            }
            let score = 0;
            for (const response of responses) {
                ...
                if (correctOptions.length !== 1) {
                    throw httpError(500, `Question ${response.questionId} must have exactly one correct option.`);
                }
            ...
            return {
                ...
                allCorrectOptions: Object.fromEntries(quiz.questions.map((question) => {
                    const correctOption = question.options.find((o) => o.isCorrect);
...
export const listAttempts = async ({ limit, offset }) => {
