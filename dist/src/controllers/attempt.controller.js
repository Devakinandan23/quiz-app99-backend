import { deleteAttempt, getAttemptById, listAttempts, submitAttempt, } from '../services/attempt.service.js';
...
export const createAttemptController = async (req, res, next) => {
    try {
...
export const getAttemptsController = async (req, res, next) => {
    try {
...
export const deleteAttemptController = async (req, res, next) => {
    try {
        const rawAttemptId = getSingleString(req.params.id);
        const attemptId = Number.parseInt(rawAttemptId ?? '', 10);
        if (!Number.isInteger(attemptId) || attemptId <= 0) {
